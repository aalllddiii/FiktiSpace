...
<body>
    ...
    <div class="table-responsive jumbotron">
        <h2 class="text-danger text-center">Data Team REJECT PUBG Mobile</h2>
        <a href="../" class="btn btn-info">⏎ Home</a>
        <a href="approve.php" class="btn btn-success">TO Approved</a>
        <a href="reject.php" class="btn btn-danger">TO Reject</a>
        <a href="../../config/reject/pubgm.php" class="btn btn-success float-right">Report Excel</a>
        <br><br>
        <table id="data-example" class="table table-striped table-bordered table-hover" style="width:100%">
            ...
            $no = 1;
            $tampil = mysqli_query($conn, "SELECT * FROM `fspace_account` WHERE `jenis_lomba` LIKE '%pubgm%' AND `validasi_payment`=0");
            ...
            ?>
                <tbody>
                    <tr>
                        ...
                        <td>
                            <a class="btn btn-success" onclick="return confirm('Approve data?')" href="../../config/update/approve/pubgm.php?id=<?php echo $data['id']; ?>" role="button">✔</a><br>
