...
$no = 1;
$tampil = mysqli_query($conn, "SELECT * FROM fspace_pubgm WHERE `username` = '$username'");
...
<body>
    ...
    <div class="table-responsive jumbotron">
        ...
        <h2 class="text-center">Data Team <b><?php echo $nama_team; ?></b> PUBG Mobile</h2>
        ...
        <a href="../../config/reject_team/pubgm.php" class="btn btn-success float-right">Report Excel</a>
        </br>
        </br>
        <table id="data-example" class="table table-striped table-bordered table-hover" style="width:100%">
            ...
            $no = 1;
            $tampil = mysqli_query($conn, "SELECT * FROM fspace_pubgm WHERE `validasi`=0 AND `nama_lengkap` <> '' AND `nama_team` = '$nama_team'");
            while ($data = mysqli_fetch_array($tampil)) {
            ...
            ?>
                <tbody>
                    <tr>
                        ...
                        <td><?php echo $data['nickid']; ?></td>
                        <td><a href="../../../data_user/<?= $data['krs'] ?>" target="__blank">KRS</a></td>
                        <td>
                            <a class="btn btn-success" onclick="return confirm('Approve data?')" href="../../config/update/approve_team/pubgm.php?id=<?php echo $data['id']; ?>&username=<?= $username ?>" role="button">✔</a><br>
