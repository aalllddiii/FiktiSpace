...
$no = 1;
$tampil = mysqli_query($conn, "SELECT * FROM fspace_mobilelegend WHERE `username` = '$username'");
...
<body>
    ...
    <div class="table-responsive jumbotron">
        ...
        <h2 class="text-center">Data Team <b><?php echo $nama_team; ?></b> Mobile Legend</h2>
        ...
        <a href="../../config/reject_team/ml.php" class="btn btn-success float-right">Report Excel</a>
        ...
        <table id="data-example" class="table table-striped table-bordered table-hover" style="width:100%">
            ...
            $no = 1;
            $tampil = mysqli_query($conn, "SELECT * FROM fspace_mobilelegend WHERE `validasi`=0 AND `nama_lengkap` <> '' AND `nama_team` = '$nama_team'");
            // $tampil = mysqli_query($conn, "SELECT * FROM fspace_mobilelegend WHERE `validasi`=0 AND `nama_lengkap` <> '' AND `instansi` <> ''");
            ...
            ?>
                <tbody>
                    <tr>
                        ...
                        <td>
                            <a class="btn btn-success" onclick="return confirm('Approve data?')" href="../../config/update/approve_team/ml.php?id=<?php echo $data['id']; ?>&username=<?= $username ?>" role="button">✔</a><br>
