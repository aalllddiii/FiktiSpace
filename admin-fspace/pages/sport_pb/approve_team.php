...
<body>
    ...
    <div class="table-responsive jumbotron">
        ...
        <a href="../../config/approve_team/pb.php" class="btn btn-success float-right">Report Excel</a>
        </br>
        </br>
        <table id="data-example" class="table table-striped table-bordered table-hover" style="width:100%">
            ...
            $no = 1;
            $tampil = mysqli_query($conn, "SELECT * FROM fspace_pointblank WHERE `validasi`=1 AND `nama_lengkap` <> '' AND `nama_team` = '$nama_team'");
            while ($data = mysqli_fetch_array($tampil)) {
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
                            ...
                            <a class="btn btn-danger" onclick="return confirm('Reject data?')" href="../../config/update/reject_team/pb.php?id=<?php echo $data['id']; ?>&username=<?= $username ?>" role="button">╳</a>
                        </td>
                        <!-- <td>
