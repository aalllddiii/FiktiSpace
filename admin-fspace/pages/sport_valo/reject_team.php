...
<body>
    ...
    <div class="table-responsive jumbotron">
        ...
        <a href="../../config/reject_team/valo.php" class="btn btn-success float-right">Report Excel</a>
        ...
        <table id="data-example" class="table table-striped table-bordered table-hover" style="width:100%">
            ...
            $no = 1;
            $tampil = mysqli_query($conn, "SELECT * FROM fspace_valorant WHERE `validasi`=0 AND `nama_lengkap` <> '' AND `nama_team` = '$nama_team'");
            while ($data = mysqli_fetch_array($tampil)) {
            ...
            ?>
                <tbody>
                    <tr>
                        ...
                        <td>
                            <a class="btn btn-success" onclick="return confirm('Approve data?')" href="../../config/update/approve_team/valo.php?id=<?php echo $data['id']; ?>&username=<?= $username ?>" role="button">✔</a><br>
