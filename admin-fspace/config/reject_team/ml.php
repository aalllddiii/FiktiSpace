...
header("Content-Disposition: attachment; filename=Data REJECT Anggota Mobile Legend.xls");
?>
...
<h3>Data REJECT Anggota Mobile Legend</h3>
...
<table id="data-example" class="table table-striped table-bordered table-hover" style="width:100%">
    ...
    $no = 1;
    $tampil = mysqli_query($conn, "SELECT * FROM fspace_mobilelegend WHERE `validasi`=0 AND `nama_lengkap` <> ''");
